...
当我们在上传像视频、包含大量图片的资料等文件时，如果不做特殊处理，可能会遇到这些问题：

1. 网络中断或程序异常退出等问题，导致文件上传失败，而不得不重新上传
2. 同一个文件被用户反复上传，从而导致服务器资源的浪费

因此需要一个针对大文件上传的方案来解决上述问题。
...
> BFF，Backend For Frontend，后端为前端服务，也就是中间层。
>
> 当公司规模到达一定程度时，就需要用到中间层，因为业务开始逐渐复杂，前后端交互的数据结构可能会出现巨大的差异并且无法抹除，这个时候就需要中间层来进行数据处理。

<img src="https://penguinbucket.obs.cn-southwest-2.myhuaweicloud.com/img/202405010933720.png" alt="image-20240312150330145" style="zoom: 33%;" />
...
<img src="https://penguinbucket.obs.cn-southwest-2.myhuaweicloud.com/img/202405010933817.png" alt="image-20240312152044755" style="zoom:25%;" />
...
> hash是一种算法，可以将任何长度的数据转换为定长的数据，常见的hash算法包括 MD5、SHA-1。
>
> 本节课使用 MD5 进行 hash 计算，使用第三方库 [Spark-MD5](https://github.com/satazor/js-spark-md5)。

<img src="https://penguinbucket.obs.cn-southwest-2.myhuaweicloud.com/img/202405010933938.png" alt="image-20240312153125355" style="zoom:25%;" />

不仅针对分片如此，针对整个文件也是如此。

<img src="https://penguinbucket.obs.cn-southwest-2.myhuaweicloud.com/img/202405010933028.png" alt="image-20240312153716061" style="zoom:33%;" />
...
<img src="https://penguinbucket.obs.cn-southwest-2.myhuaweicloud.com/img/202405010933109.png" alt="image-20240312160059174" style="zoom:40%;" />

为了解决这个问题，我们可以对大文件上传做一个大胆的假设：绝大部分的文件上传都是新文件上传。

有了这个假设，我们就无须等待整体 hash 的计算结果，直接上传分片即可，同时可以把分片操作使用多线程+异步的方式进行上传处理。

<img src="https://penguinbucket.obs.cn-southwest-2.myhuaweicloud.com/img/202405010933203.png" alt="image-20240312160813531" style="zoom:40%;" />
...
创建文件协议就是用于获取文件上传的唯一标识。（此处最好时 get 请求）

<img src="https://penguinbucket.obs.cn-southwest-2.myhuaweicloud.com/img/202405010933316.png" alt="image-20240313083216884" style="zoom:25%;" />
...
<img src="https://penguinbucket.obs.cn-southwest-2.myhuaweicloud.com/img/202405010933344.png" alt="image-20240313084315768" style="zoom:30%;" />
...
<img src="https://penguinbucket.obs.cn-southwest-2.myhuaweicloud.com/img/202405010933420.png" alt="image-20240313102553900" style="zoom:30%;" />

#### 分片合并协议

当所有的分片全部上传后，通过此协议请求服务器完成分片合并。

<img src="https://penguinbucket.obs.cn-southwest-2.myhuaweicloud.com/img/202405010933717.png" alt="image-20240313102931840" style="zoom:25%;" />

### 代码如何组织？

大文件上传 SDK 的搭建分为三层：

<img src="https://penguinbucket.obs.cn-southwest-2.myhuaweicloud.com/img/202405010933716.png" alt="image-20240308093004656" style="zoom:30%;" />
...
<img src="https://penguinbucket.obs.cn-southwest-2.myhuaweicloud.com/img/202405010933715.png" alt="image-20240313134138969" style="zoom:33%;" />
...
<img src="https://penguinbucket.obs.cn-southwest-2.myhuaweicloud.com/img/202405010933763.png" alt="image-20240313135216753" style="zoom:33%;" />
